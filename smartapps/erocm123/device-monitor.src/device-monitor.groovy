...
def pageStatus(params) {
	...
	return dynamicPage(pageProperties) {
            ...
            section("Menu") {
                href "pageStatus", title:"Refresh", description:"Tap to refresh the status of devices", params: [refresh: true]
...
def doCheck () {
    ...
    lockdevices, alarmdevices, switchdevices, presencedevices, smokedevices].each { n ->
       if (n != null) allDevices += n
    }
    
    if (allDevices != null) {
    def allDevicesUnique = allDevices.unique( { a, b -> a["id"] <=> b["id"] } ).sort( {a, b -> a["displayName"] <=> b["displayName"] } )
			...
			try {
				if (lastTime) {
                    ...
                    if (xhours > timer){
                        ...
                        delaylistMap += [[time:"$xhours", name:"$it.displayName"]]
                        delaylistCheckMap += [[name:"$it.displayName"]]
                    }
                    goodlistMap += [[time:"$xhours", name:"$it.displayName"]]
				} else {
					badlistMap += [[name:"$it.displayName"]]
				}

			} catch (e) {
					log.trace "Caught error checking a device."
					log.trace e
					errorlistMap += [[name:"$it.displayName"]]
			}
		}
        
        def goodlistUniqueSorted = goodlistMap.sort( {a, b -> b["time"] as float <=> a["time"] as float } )
        ...
        def delaylistUniqueSorted = delaylistMap.sort( {a, b -> b["time"] as float <=> a["time"] as float } )
        delaylistUniqueSorted.each { 
           delaylist += "${it.time} - ${it.name}\n"
            ...
            if ((badlistMapDiff || errorlistMapDiff || delaylistCheckMapDiff) && ((location.contactBookEnabled && recipients) || (sendPushMessage != "No") || (phoneNumber != "0")))
            {   
            ...
            def notifications = []
            	...
            	if(delaylistCheckMapDiff)
                {
                    ...
                    def newMap = []
                    delaylistCheckMapDiff.each { n ->
                       newMap += delaylistUniqueSorted.find { it.name == n.name }
                    }
                    newMap.sort( {a, b -> b["time"] as float <=> a["time"] as float } ).each { 
...
//***************************
def pageConfigure() {
	def helpPage = "Select devices that you wish to check when you open this SmartApp. These devices will also get checked at the desired intervals and notifications will be sent based on the settings below"
...
def subscribeDevices(){
    ...
    subscribe(motiondevices, "motion", eventCheck)
    subscribe(humiditydevices, "relativeHumidity", eventCheck)
    subscribe(leakdevices, "water", eventCheck)
    subscribe(thermodevices, "Temperature", eventCheck)
    subscribe(thermodevices, "heatingSetpoint", eventCheck)
    subscribe(thermodevices, "coolingSetpoint", eventCheck)
    subscribe(tempdevices, "temperature", eventCheck)
    subscribe(contactdevices, "contact", eventCheck)
    subscribe(lockdevices, "lock", eventCheck)
    subscribe(alarmdevices, "alarm", eventCheck)
    subscribe(switchdevices, "switch", eventCheck)
    subscribe(presencedevices, "presence", eventCheck)
    subscribe(smokedevices, "smokeDetector", eventCheck)
